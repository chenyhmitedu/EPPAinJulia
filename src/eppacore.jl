...
function EPPACore(MGE, data, setting)
    ...
    @sectors(MGE, begin
        D[g=data["set_g"], r=data["set_r"]],                       (description = "Supply")
    ...
    @commodities(MGE, begin
        PD[g=data["set_g"], r=data["set_r"]],                      (description = "Domestic output price")
    ...
    #for i ∈ data["set_i"], g ∈ data["set_g"], r ∈ data["set_r"]
        @production(MGE, A[i=data["set_i"], g=data["set_g"], r=data["set_r"]], [t = 0, s = data["esubd"][i]], begin
            @output(PA[i, g, r],                    data["xa0"][r, i, g],   t)
            @input(PD[i, r],                        data["xd0"][r, i, g],   s,   taxes = [Tax(RA[r], rtfd[i, g, r])],   reference_price = 1 + data["rtfd0"][i, g, r])
            @input(PM[i, r],                        data["xm0"][r, i, g],   s,   taxes = [Tax(RA[r], rtfi[i, g, r])],   reference_price = 1 + data["rtfi0"][i, g, r])  
        end)
    #end

    #for g ∈ data["set_i"], r ∈ data["set_r"]
        @production(MGE, D[g=data["set_i"], r=data["set_r"]], [t = data["etadx"][g], s = data["esub"][g], sn => s = data["esubn"][g], sve => sn = data["esubve"][g], sva => sve = data["esubva"][g], sef => sve = data["esubef"][g], sf => sef = data["esubf"][g]], begin
            @output(PD[g, r],                       data["vom"][g, r], t, taxes = [Tax(RA[r], td[r, g])], reference_price = 1-data["rto0"][g, r])    
    ...
    for g ∈ data["set_cgi"], r ∈ data["set_r"]
        @production(MGE, D[g, r], [t = 0, s = data["esub"][g], sn => s = data["esubn"][g], sef => sn = data["esubef"][g], sf => sef = data["esubf"][g]], begin
            @output(PD[g, r],                       data["vom"][g, r], t, taxes = [Tax(RA[r], td[r, g])])
            @input(PA[i=data["set_fe"], g, r],      data["vafm"][i, g, r],  sf)
            @input(PA[i=data["set_elec"], g, r],    data["vafm"][i, g, r],  sef)
            @input(PA[i=data["set_ne"], g, r],      data["vafm"][i, g, r],  sn)
        end)
    ...
    #for i ∈ data["set_i"], s ∈ data["set_r"], r ∈ data["set_r"]
        @production(MGE, E[i=data["set_i"], s=data["set_r"], r=data["set_r"]], [t = 0, s = 0], begin
            ...
            @input(PD[i, s],                        data["vxmd"][i, s, r], s,   taxes = [Tax(RA[s], -rtxs[i, s, r])],   reference_price = 1 - data["rtxs0"][i, s, r])
    ...
    #for r ∈ data["set_r"] 
        @demand(MGE, RA[r=data["set_r"]], begin
            @final_demand(PD[:c, r],     data["vom"][:c, r])
            @endowment(PD[:c, :USA],     data["vb"][r])
            @endowment(PD[:g, r],        -data["vom"][:g, r])
            @endowment(PD[:i, r],        -data["vom"][:i, r])
